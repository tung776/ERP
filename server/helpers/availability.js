...
module.exports = function(models) {
  "use strict";
  return new function() {
    var AvailabilityService = require("../services/productAvailability")(
      models
    );
                    ...
                    function(avalability, cb) {
                      ...
                      if (orderRow.orderRowId) {
                        avalability.orderRows.forEach(function(orderRowEl) {
                          if (
                            orderRowEl.orderRowId &&
                            orderRowEl.orderRowId.toJSON() ===
                              orderRow.orderRowId.toJSON()
                          ) {
                            existedRow = orderRow;
                          }
                        });
                      ...
                      function callback() {
                        ...
                        if (orderRow.locationsDeliver && avalability.location) {
                          locationsDeliverIds = orderRow.locationsDeliver.map(
                            function(elem) {
                              return elem.toString();
                            }
                          );
                          if (
                            !locationsDeliverIds.length ||
                            locationsDeliverIds.indexOf(
                              avalability.location._id.toString()
                            ) === -1
                          ) {
                            orderRow.locationsDeliver.push(
                              avalability.location._id
                            );
                          }
                        }
                        ...
                        if (orderRow.batchesDeliver) {
                          existedBatch = _.find(
                            orderRow.batchesDeliver,
                            function(el) {
                              return (
                                el.goodsNote.toString() ===
                                avalability.goodsInNote.toString()
                              );
                            }
                          );
                          if (existedBatch) {
    ...
    this.deliverProducts = function(options, mainCb) {
      ...
      AvailabilityService.updateByQuery(
        ...
        function(err) {
          ...
          async.each(
            goodsOutNote.orderRows,
            function(orderRow, cb) {
              var accountsItems = [];
              ...
              OrderRows.populate(
                ...
                function(err) {
                  var debitAccount = orderRow.orderRowId
                    ? orderRow.orderRowId.debitAccount
                    : null;
                  var creditAccount = orderRow.orderRowId
                    ? orderRow.orderRowId.creditAccount
                    : goodsOutNote.warehouse
                    ? goodsOutNote.warehouse.account
                    : null;
                          ...
                          if (!err) {
                            body.accountsItems.push(
                              ...
                              },
                              {
                                ...
                                debit: 0,
                                account:
                                  goodsOutNote.order.paymentMethod.chartAccount
                              }
                            );
    ...
    this.receiveProducts = function(options, mainCb) {
      ...
      var dbName = options.dbName;
      var warehouseTo = goodsInNote.warehouseTo
        ? goodsInNote.warehouseTo._id
        : goodsInNote.warehouse;
      var uId = options.uId;
      var body;
      ...
      async.each(
        goodsInNote.orderRows,
        function(elem, eachCb) {
          ...
          function createEntries(parallelCb) {
            ...
            OrderRows.populate(
              ...
              function(err) {
                var debitAccount = elem.orderRowId
                  ? elem.orderRowId.debitAccount
                  : goodsInNote.warehouseTo
                  ? goodsInNote.warehouseTo.account
                  : "";
                var creditAccount = elem.orderRowId
                  ? elem.orderRowId.creditAccount
                  : null;
                      ...
                      if (!err) {
                        body.accountsItems.push(
                          ...
                          },
                          {
                            ...
                            debit: 0,
                            account:
                              goodsInNote.order.paymentMethod.chartAccount
                          }
                        );
