...
var History = function(models) {
  ...
  this.sendToFollowers = function(options, callback) {
    ...
    function getEmails(query, history, cb) {
      FollowerService.find(query, { dbName: dbName })
    ...
    function sendTo(history, employees, cb) {
      ...
      OrgSettingsService.getFromMail({ dbName: dbName }, function(
        err,
        settings
      ) {
        if (err) {
        ...
        async.each(
          ...
          function(empObject, asyncCb) {
            ...
            if (historyEntry.editedBy) {
              isMe =
                historyEntry.editedBy._id.toString() ===
                empObject._id.toString();
            } else {
              if (historyEntry.authorId) {
                isMe = historyEntry.authorId === empObject._id.toString();
              } else {
                isMe = historyEntry.user
                  ? historyEntry.user._id === empObject._id.toString()
                  : false;
              }
            }
