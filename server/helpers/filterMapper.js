...
var FilterMapper = function() {
  ...
  function convertType(values, type, operator) {
    ...
    switch (type) {
      ...
      case "letter":
        result = new RegExp(
          "^[" + values.toLowerCase() + values.toUpperCase() + "].*"
        );
        break;
    }
  ...
  this.mapFilter = function(filter, options) {
    ...
    if (fieldsArray && Array.isArray(fieldsArray)) {
      filterNames = withoutState
        ? _.difference(filterNames, fieldsArray)
        : fieldsArray;
    }
    ...
    for (i = filterNames.length - 1; i >= 0; i--) {
      ...
      if (filterNames.indexOf(filterName) !== -1) {
        ...
        filterType = !!filterObject.type
          ? filterObject.type
          : filterConstantsByName.type || "ObjectId";
        filterBackend =
          filterConstantsByName.backend ||
          filterObject.key ||
          filterObject.backend;

        if (
          (contentType === "GoodsOutNote" ||
            contentType === "stockTransactions") &&
          filterBackend === "status"
        ) {
          ...
          if (filterBackend) {
            if (typeof filterBackend === "string") {
              ...
              _.map(filterBackend, function(keysObject) {
                ...
                resObj[keysObject.key] = convertType(
                  filterValues,
                  filterType,
                  keysObject.operator
                );
