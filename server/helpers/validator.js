...
function parseTaskBody(body) {
  ...
  if (body.attachments) {
    ...
    if (attachments.uploaderName) {
      ...
      attachments.uploaderName = xssFilters.inHTMLData(
        attachments.uploaderName
      );
    }
  } //unused
  if (body.notes) {
  ...
  if (body.estimated) {
    ...
    body.EndDate = dateCalculator.calculateTaskEndDate(
      StartDate,
      body.estimated
    );
...
function validEmployeeBody(body) {
  ...
  var hasNameFirst = hasName
    ? validator.isEmployeeName(body.name.first)
    : false;
