...
eventsHandler = function(app, mainDb) {
  ...
  event.on("removeWorkflow", function(req, wId, id) {
    ...
    if (query) {
      query
        .update({ workflow: id }, { workflow: null }, { multi: true })
        .exec(function(err) {
          if (err) {
            logWriter.error(err);
          }
  ...
  event.on("setReconcileTimeCard", function(options) {
    ...
    if (jobs) {
      jobsModel.update({ _id: jobs }, { $set: { reconcile: true } }, function(
        err,
        result
      ) {
        if (err) {
              ...
              GoodsOutNote.aggregate(
                ...
                function(err, docs) {
                  ...
                  if (!docs || !docs.length) {
                    ...
                    stockStatus.fulfillStatus =
                      stockStatus.fulfillStatus === "NOA" ||
                      stockStatus.fulfillStatus === "ALL"
                        ? "NOA"
                        : "NOT";
                    stockStatus.shippingStatus =
                      stockStatus.shippingStatus === "NOA" ||
                      stockStatus.shippingStatus === "ALL"
                        ? "NOA"
                        : "NOT";
                  } else {
                    ...
                    if (fullfillOnRow !== elem.quantity) {
                      ...
                      stockStatus.fulfillStatus =
                        stockStatus.fulfillStatus === "NOA" ? "NOA" : "ALL";
                    }
                  }
  ...
  event.on("updateName", function(
    ...
    targetModel.update(sercObject, updateObject, { multi: true }, function(
      err
    ) {
      if (err) {
        logWriter.error(err);
  ...
  event.on("updateSequence", function(
    model,
    sequenceField,
    start,
    end,
