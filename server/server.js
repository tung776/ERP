...
const uri = `mongodb://${process.env.MAIN_DB_HOST}:${process.env.DB_PORT}/${
  process.env.MAIN_DB_NAME
}`;
mainDb = mongoose.createConnection(uri, connectOptions);
mongoose.connect(
    uri,
    function callback() {
        ...
        main.find().exec(function(err, result) {
            ...
            async.each(
                ...
                function(_db, eachCb) {
                    ...
                    //   var dbObject = mongoose.connect(
                    //     uriDbObject,
                    //     connectOptions
                    //   );
                    var dbObject = mongoose.createConnection(uriDbObject, connectOptions);
                    mongoose.connect(
                        uriDbObject,
                        () => {
