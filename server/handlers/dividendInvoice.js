...
var Proforma = function(models) {
  ...
  this.create = function(req, res, next) {
    ...
    var DividendInvoice = models.get(
      dbIndex,
      "dividendInvoice",
      DividendInvoiceSchema
    );
    var request;
    ...
    function getRates(callback) {
      ratesService.getById({ id: date, dbName: req.session.lastDb }, function(
        err,
        result
      ) {
    ...
    function createDividendInvoice(parallelResponse, callback) {
      ...
      saveObject.currency.rate = ratesRetriever.getRate(
        fx.rates,
        fx.base,
        data.currency._id
      );
    ...
    function createJournalEntry(dividendInvoice, callback) {
      ...
      _journalEntryHandler.createReconciled(body, {
