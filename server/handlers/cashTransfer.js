...
var cashTransfer = function(models, event) {
  ...
  this.create = function(req, res, next) {
    var cashTransferModel = models.get(
      req.session.lastDb,
      "cashTransfer",
      cashTransferSchema
    );
    ...
    ratesService.getById(query, function(err, ratesObject) {
      ...
      cashTransfer.save(function(err, result) {
        ...
        bodyJournalEntry = {
          // ToDo Cash Transefer between different accounts
          currency: {
            name: currencyTo,
            rate: currencyRateTo
          },
        ...
        JournalEntryService.createMultiRows(bodyJournalEntry, function(
          err,
          result
        ) {
          if (err) {
  ...
  this.patchM = function(req, res, next) {
    ...
    async.each(
      ...
      function(data, cb) {
        ...
        delete data._id;
        cashTransferModel.findByIdAndUpdate(
          id,
          { $set: data },
          { new: true },
          cb
    ...
    async.each(
      ...
      function(id, cb) {
        cashTransferModel.remove({ _id: id }, function(err, removed) {
          ...
          JournalEntryService.remove(
            { dbName: req.session.lastDb, query: { "sourceDocument._id": id } },
            cb
          );
        });
      },
