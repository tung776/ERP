...
var Module = function(models, event) {
  ...
  function getFromMail(mailOptions, cb) {
    var OrgSettings;
    if (mailOptions.req) {
      OrgSettings = models.get(
        mailOptions.req.session.lastDb,
        "orgSettings",
        OrgSettingsSchema
      );
      OrgSettings.findOne()
  ...
  function getTasksForKanban(req, res, next) {
    ...
    var limit;
    var pipeline;
    var worflowObj = data.workflowId
    ...
    if (filter) {
      ...
      optionsObject.$and.push(
        filterMapper.mapFilter(filter, { contentType: dealTasksCT })
      );
    }
  ...
  function getTasksForList(req, res, next) {
    ...
    if (filter) {
      obj.$and = [];
      obj.$and.push(
        filterMapper.mapFilter(data.filter, { contentType: dealTasksCT })
      );
    }
              ...
              $filter: {
                ...
                cond: {
                  $and: [
                    ...
                    { $ne: ["$$task.workflow.status", "Cancelled"] }
                  ]
                }
              }
            },
  ...
  this.uploadFile = function(req, res, next) {
    ...
    uploader.postFile(dir, files, { userId: req.session.uName }, function(
      err,
      file
    ) {
      if (err) {
      ...
      Model.findByIdAndUpdate(
        ...
        function(err, response) {
          ...
          res
            .status(200)
            .send({ success: "Tasks updated success", data: response });
        }
      );
    });
  };
  ...
  this.taskUpdateOnlySelectedFields = function(req, res, next) {
    ...
    function updateTask() {
      models
        .get(dbName, "DealTasks", tasksSchema)
        .findByIdAndUpdate(_id, { $set: data }, { new: true }, function(
          err,
          result
        ) {
          var historyOptions;
    ...
    function sequenceUpdate() {
      if (data.sequence === -1) {
        event.emit(
          "updateSequence",
          models.get(dbName, "DealTasks", tasksSchema),
          "sequence",
          data.sequenceStart,
          data.sequence,
          data.workflowStart,
  ...
  this.getActivity = function(req, res, next) {
    ...
    Task.find(obj, { _id: 1, description: 1 }, function(err, docs) {
      ...
      HistoryService.getHistoryForTrackedObject(historyOptions, function(
        err,
        history
      ) {
        if (err) {
    ...
    async.each(
      ...
      function(id, cb) {
        Model.findByIdAndRemove(id, function(err, task) {
          ...
          event.emit(
            "updateSequence",
            models.get(req.session.lastDb, "DealTasks", tasksSchema),
            "sequence",
            task.sequence,
            0,
            task.workflow,
  ...
  this.getTasksPriority = function(req, res, next) {
    models
      .get(req.session.lastDb, "Priority", prioritySchema)
      .find({}, function(err, _priority) {
        if (err) {
          return next(err);
        }
