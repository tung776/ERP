  ...
  this.composeForVacation = function(req, res, next) {
    ...
    if (filter.salesManager && filter.salesManager.value) {
      ...
      if (filter.salesManager.value.length) {
        filterElement[filter.salesManager.key] = {
          $in: filter.salesManager.value.objectID()
        };
    ...
    function resultMapper(err, result) {
      ...
      function departmentMapper(department, departmentCb) {
        var dashDepartment = _.find(dashBoardResult, function(deps) {
          return (
            deps.department.toString() === department.department.toString()
          );
        });

        async.each(
          department.employees,
          function(_employee, employeeCb) {
            ...
            function holidaysVacationComposer(_employee, dashResultByEmployee) {
              _employee.weekData = _.map(tempWeekArr, function(weekData) {
                ...
                data = dashResultByEmployee
                  ? _.find(dashResultByEmployee.weekData, function(d) {
                      return (
                        parseInt(d.dateByWeek, 10) ===
                        parseInt(weekData.dateByWeek, 10)
                      );
                    })
                  : null;
                ...
                _vacations = _.find(vacations, function(vacationObject) {
                  ...
                  return (
                    employee &&
                    employee._id.toString() === _employee._id.toString()
                  );
                });
            ...
            if (dashDepartment) {
              dashResultByEmployee = _.find(
                dashDepartment.employeeData,
                function(employeeData) {
                  return (
                    employeeData.employee.toString() ===
                    _employee._id.toString()
                  );
                }
              );
            ...
            _employee.maxProjects = dashResultByEmployee
              ? dashResultByEmployee.maxProjects
              : 0;
    ...
    async.waterfall(
      [
        function(wfCb) {
          ...
          Department.find(departmentQuery, { _id: 1 })
            .sort({
              nestingLevel: 1,
              sequence: -1
            })
        ...
        function(depIds, wfCb) {
              ...
              [
                  ...
                  $project: {
                    ...
                    lastHire: {
                      $let: {
                        vars: {
                          lastHired: {
                            $arrayElemAt: [{ $slice: ["$hire", -1] }, 0]
                          }
                        },

                        in: {
                          $add: [
                            { $multiply: [{ $year: "$$lastHired" }, 100] },
                            { $week: "$$lastHired" }
                          ]
                        }
                      }
                ...
                },
                {
                  $group: {
                    ...
                    name: {
                      $first: { $concat: ["$name.first", " ", "$name.last"] }
                    },
                ...
                },
                {
                  $project: {
                    ...
                    name: 1,
                    lastTransferObject: {
                      $arrayElemAt: [{ $slice: ["$isTransfer", -1] }, 0]
                    },
                    _lastTransferDate: {
                      $add: [
                        { $multiply: [{ $year: "$lastTransferDate" }, 100] },
                        { $week: "$lastTransferDate" }
                      ]
                    },
                    _firstTransferDate: {
                      $add: [
                        { $multiply: [{ $year: "$firstTransferDate" }, 100] },
                        { $week: "$firstTransferDate" }
                      ]
                    }
                  }
                },
                {
                  $project: {
                    ...
                    lastTransferObjectDate: {
                      $add: [
                        {
                          $multiply: [
                            { $year: "$lastTransferObject.date" },
                            100
                          ]
                        },
                        { $week: "$lastTransferObject.date" }
                      ]
                    }
                  }
                },
          ...
          function dashComposer(parallelCb) {
            function employeeFinder(waterfallCb) {
              ...
              async.waterfall([groupWtrackByEmployee, findEmployee], function(
                err,
                result
              ) {
                if (err) {
            ...
            function wTrackComposer(employeesArray, waterfallCb) {
              var salesManagerMatch = {
                $and: [
                  {
                    $eq: [
                      "$$projectMember.projectPositionId",
                      objectId(CONSTANTS.SALESMANAGER)
                    ]
                  },
                  {
                    $or: [
                            ...
                            $lte: [
                              {
                                $add: [
                                  {
                                    $multiply: [
                                      { $year: "$$projectMember.startDate" },
                                      100
                                    ]
                                  },
                                  { $week: "$$projectMember.startDate" }
                                ]
                              },
                              "$_id.dateByWeek"
                            ]
                            ...
                            $gte: [
                              {
                                $add: [
                                  {
                                    $multiply: [
                                      { $year: "$$projectMember.endDate" },
                                      100
                                    ]
                                  },
                                  { $week: "$$projectMember.endDate" }
                                ]
                              },
                              "$_id.dateByWeek"
                            ]
            ...
            async.waterfall(
              [employeeFinder, wTrackComposer],
              masterWaterfallCb
            );
          }

          function holidaysComposer(parallelCb) {
            var Holiday = models.get(
              req.session.lastDb,
              "Holiday",
              HolidaySchema
            );
        ...
        [
          ...
          },
          {
            $project: {
              ...
              hireDate: {
                $add: [
                  { $multiply: [{ $year: "$firstHiredDate" }, 100] },
                  { $month: "$firstHiredDate" }
                ]
              },
              name: 1
            }
