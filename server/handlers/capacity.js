...
var Module = function(models) {
  ...
  function getCapacityFilter(modelId, req, res, next) {
    ...
    query = Capacity.find(queryObject).populate("vacation");
    ...
    query.exec(function(err, result) {
      ...
      res
        .status(200)
        .send({ capacityObject: result, departmentObject: departmentsObject });
    });
  }
  ...
  this.putchModel = function(req, res, next) {
    ...
    Capacity.findByIdAndUpdate(id, { $set: data }, { new: true }, function(
      err,
      response
    ) {
      if (err) {
