...
var User = function(event, models) {
  ...
  function checkIfUserLoginUnique(req, login, cb, db) {
    ...
    models
      .get(db, "Users", userSchema)
      .find({ login: login }, function(error, doc) {
        if (error) {
          return cb(error);
        }
          ...
          .findById(_id, { savedFilters: 1 }, function(err, result) {
            ...
            savedFilters = _.map(savedFilters, function(element) {
              if (element.contentType === contentType) {
                element.byDefault =
                  byDefaultUpdate &&
                  byDefaultUpdate._id === element._id.toString()
                    ? byDefaultUpdate.byDefault
                    : false;
              }
  ...
  function createSession(options, cb) {
    ...
    if (
      !_user ||
      !_user._id ||
      (checkPass && _user.pass !== shaSum.digest("hex"))
    ) {
  ...
  this.forgotPassword = function(req, res, next) {
    ...
    function findUser(waterfallCb) {
      UserModel.findOne(queryObject, { login: 1, email: 1 }, function(
        err,
        _user
      ) {
        if (err) {
    ...
    async.waterfall(
      [findUser, paswordGenerator, sendMail, updateUser],
      function(err) {
        if (err) {
          return next(err);
        }
  ...
  this.create = function(req, res, next) {
    ...
    UserService.create(body, function(err, user) {
      ...
      res
        .status(201)
        .send({ success: "A new User crate success", id: user._id });
    });
  };
  ...
  this.verify = function(req, res, next) {
    ...
    UserService.findOneAndUpdate(
      ...
      function(err, result) {
        if (
          err ||
          (result && result.credentials && result.credentials.verify_token)
        ) {
      ...
      function(err, result) {
        ...
        if (result) {
          ...
          UserService.createWithVerify(body, function(err, user) {
            ...
            res
              .status(201)
              .send({ success: "A new User create success", id: user._id });
          });
        } else {
    ...
    UserService.findByIdAndRemove(
      ...
      function(err, _user) {
        ...
        if (_user && _user.relatedEmployee) {
          ...
          EmployeeService.findByIdAndUpdate(
            _id,
  ...
  this.getByProfile = function(req, res, next) {
    ...
    UserModel.find({ profile: profileId }, { _id: 0, login: 1 }, function(
      err,
      result
    ) {
      if (err) {
  ...
  this.getForDd = function(req, res, next) {
    ...
    findUsers(UserModel, { login: 1, email: 1 }, { login: 1 }, data, function(
      err,
      users
    ) {
      if (err) {
  ...
  this.getById = function(req, res, next) {
    ...
    UserModel.aggregate(pipeLine).exec(function(err, result) {
      var savedFilters;
